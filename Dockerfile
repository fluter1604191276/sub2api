...
# These values bind the runtime image to the exact source snapshot used to
# build it. The production release gate verifies them before a switch.
ARG VERSION=
ARG COMMIT=
ARG DATE=
ARG SOURCE_SNAPSHOT=

# Re-declare build metadata in the final stage so BuildKit can persist it as
# OCI labels instead of keeping it scoped to the builder stage.
ARG VERSION
ARG COMMIT
ARG DATE
ARG SOURCE_SNAPSHOT
...
LABEL org.opencontainers.image.source-snapshot="${SOURCE_SNAPSHOT}"
