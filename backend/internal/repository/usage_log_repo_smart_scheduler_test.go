...
func TestUsageLogRepositoryGetSmartSchedulerErrorStatsBatch(t *testing.T) {
	...
	}).AddRow(int64(11), int64(90), int64(2), int64(3), int64(4), int64(5), int64(6), int64(7), int64(1), int64(2), int64(3))

	mock.ExpectQuery(`(?s)WITH successful AS .*FROM usage_logs.*actual_cost > 0.*ul\.stream = TRUE.*classified AS .*FROM ops_error_logs.*oe\.stream = TRUE.*failures AS .*FULL OUTER JOIN failures`).
		WithArgs(pq.Array(accountIDs), start, end).
		WillReturnRows(rows)

	repo := newUsageLogRepositoryWithSQL(nil, db)
	stats, err := repo.GetSmartSchedulerErrorStatsBatch(context.Background(), accountIDs, start, end)
	require.NoError(t, err)
...
func TestUsageLogRepositoryGetSmartSchedulerErrorStatsBatchEmpty(t *testing.T) {
	...
	repo := newUsageLogRepositoryWithSQL(nil, db)
	stats, err := repo.GetSmartSchedulerErrorStatsBatch(context.Background(), nil, time.Time{}, time.Now())
	require.NoError(t, err)
