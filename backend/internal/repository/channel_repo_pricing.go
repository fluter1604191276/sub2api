...
func (r *channelRepository) ApplyModelCalibration(ctx context.Context, updates []service.ChannelPricingModelsUpdate) error {
	...
	return r.runInTx(ctx, func(tx *sql.Tx) error {
		for _, update := range updates {
			if update.ChannelID <= 0 || update.PricingID <= 0 || len(update.PreviousModels) == 0 || len(update.Models) == 0 {
			...
			previousModelsJSON, err := json.Marshal(update.PreviousModels)
			if err != nil {
				return fmt.Errorf("marshal previous models for pricing %d: %w", update.PricingID, err)
			}
			result, err := tx.ExecContext(ctx,
				`UPDATE channel_model_pricing
				 SET models = $1, updated_at = NOW()
				 WHERE id = $2 AND channel_id = $3 AND models = $4::jsonb`,
				string(modelsJSON), update.PricingID, update.ChannelID, string(previousModelsJSON),
			)
			if err != nil {
			...
			if rows != 1 {
				return fmt.Errorf("pricing entry changed or not found during calibration: channel=%d pricing=%d", update.ChannelID, update.PricingID)
			}
		}
		return nil
	})
