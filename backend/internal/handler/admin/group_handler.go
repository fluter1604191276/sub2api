...
func NewGroupHandler(adminService service.AdminService, dashboardService *service.DashboardService, groupCapacityService *service.GroupCapacityService, smartScheduler *service.SmartSchedulerPreviewService, recoveryProbeBilling *service.GroupRecoveryProbeBillingService, configs ...*config.Config) *GroupHandler {
	h := &GroupHandler{
...
func NewGroupHandlerWithConfig(adminService service.AdminService, dashboardService *service.DashboardService, groupCapacityService *service.GroupCapacityService, cfg *config.Config) *GroupHandler {
	return NewGroupHandler(adminService, dashboardService, groupCapacityService, nil, nil, cfg)
}
...
// GetGroupModelAllowlistCandidates is the renamed-route compatibility alias.
// Keep the old handler name available for clients/tests while the public route
// uses the official model-allowlist terminology.
