...
func TestAPIKeyService_RejectsV18AuthSnapshotWithoutSmartSchedulerFlag(t *testing.T) {
	svc := &APIKeyService{}

	apiKey, ok, err := svc.applyAuthCacheEntry("k-legacy-smart-scheduler", &APIKeyAuthCacheEntry{
	...
	if ok {
		t.Fatal("expected v18 auth snapshot to be rejected after smart scheduler flag was added")
	}
	if apiKey != nil {
		t.Fatalf("expected no API key from stale snapshot, got %#v", apiKey)
	}
