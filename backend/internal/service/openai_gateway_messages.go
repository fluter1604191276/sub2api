...
func (s *OpenAIGatewayService) handleAnthropicBufferedStreamingResponse(
	...
	finalResponse, usage, acc, webSearchCalls, err := s.readOpenAICompatBufferedTerminal(resp, c, "openai messages buffered", requestID)
	if err != nil {
	...
	for {
		...
		case ev, ok := <-events:
				...
				if frame, ok := parser.Finish(); ok {
					...
					if err := json.Unmarshal([]byte(payload), &event); err == nil {
						...
						if response := openAICompatTerminalResponse(&event, []byte(payload)); isOpenAICompatResponsesTerminalEvent(event.Type) && response != nil {
							...
							return response, usage, acc, webSearchUsage.Count(), nil
						}
					}
				}
			...
			if ev.err != nil {
				...
				return nil, usage, acc, webSearchUsage.Count(), &openAICompatBufferedReadError{cause: ev.err}
			}
