...
import type { Account, AccountPlatform, AccountQualityStats, AccountSchedulerGroupScore, AccountType, Proxy as AccountProxy, AdminGroup, WindowStats, CacheHitStats, ClaudeModel, UpstreamBillingProbeSnapshot, AccountUsageInfo } from '@/types'
...
const refreshAccountQualityBatch = async () => {
  ...
  try {
    ...
    qualityStatsByAccountId.value = result?.stats ?? {}
  } catch (error) {
    if (reqSeq !== qualityStatsReqSeq.value) return
...
const loadAccountModelOptions = async () => {
  if (typeof adminAPI.accounts.listSyncedModels !== 'function') {
    accountModelOptions.value = []
    return
  }
  const models = await adminAPI.accounts.listSyncedModels()
  accountModelOptions.value = (Array.isArray(models) ? models : [])
...
onMounted(async () => {
  ...
  const syncedModelsPromise = typeof adminAPI.accounts.listSyncedModels === 'function'
    ? adminAPI.accounts.listSyncedModels()
    : Promise.resolve([])
  ...
  if (modelsResult.status === 'fulfilled') {
    ...
    console.error('Failed to load account model options:', modelsResult.reason)
  }
