export default {
    ...
    groups: {
      ...
      quality: {
        loadFailed: '加载失败',
        hint: '基于最近 24 小时归属于该分组、已计费且排除探测的流式流水，分别取最新 10 次和 100 次有耗时记录。评分 v3 先对每个分项按中位数 80%、尾部 20% 混合，再按首字 70%、生成 tokens/s 30% 合成；输出 token 少于 32 或生成时长不足 1000ms 的流水不计入 TPS 样本。账号、分组和调度评分使用同一数据公式，但因各自数据范围不同，结果可以不同。界面仍显示原始平均首字与总耗时。缺少 TPS 时使用最高 79 分的首字保守回退并明确标记；仅有生成 TPS 时最高 69 分。评级和评分仅用于展示，不会自动改变分组路由。',
        realtimeHint: '仅统计最近 1 小时归属于该分组的流式请求，并展示成功、失败和最近成功时间。“未参与”表示最近 1 小时没有真实流量，不等于分组故障；此时 24 小时评分只作为灰色历史基线。'
      },
      smartScheduler: {
        action: '智能调度',
        title: '智能调度预览 · {name}',
        description: '预览本组账号池和评分。评分综合近期质量、稳定质量、错误归因、成本倍率和实时负载。',
        enabledStatus: '本组已启用智能调度并接管生产路由。',
        disabledStatus: '本组使用原调度。',
        enableAction: '启用智能调度',
        ...
        recoveryProbe: {
          ...
          probesPerRound: '每轮次数',
          smartBackoffMax: '智能回退上限（秒）',
          idleThreshold: '空闲阈值',
          idleThresholdFixed: '固定 1h',
          noTestModel: '未配置',
          summary: '{status} · {mode} · 模型 {model} · 间隔 {interval} · 每轮 {count} 次 · 回退上限 {backoff}',
          saveSuccess: '恢复探针配置已保存',
          accountStatus: '账号探针状态',
          noAccountStatus: '暂无探针状态',
          accountModel: '模型 {model}',
          accountAttempts: '连续成功 {successes} / 失败 {failures}，累计 {total} 次',
          accountLatency: '耗时 {latency}',
          accountLastProbe: '上次 {time}',
          accountNextProbe: '下次 {time}',
          accountError: '{class}: {error}',
          unknownErrorClass: '未知错误',
          billing: {
            title: '探针费用记账',
            description: '把探针实际消耗按上游账号成本记入你自己的站内账号流水，并用全局每日预算限制支出。专用 Key 可以保持停用，不参与外部请求。',
            enabled: '已记入我的账号',
            disabled: '不记账',
            apiKey: '记账 API Key',
            selectAPIKey: '选择我名下的 Key',
            dailyBudget: '每日预算（USD）',
            perAttemptLimit: '单次预留上限（USD）',
            globalToday: '今日已结算费用',
            groupToday: '本分组已结算费用',
            budgetUsed: '今日预算占用',
            remaining: '今日剩余预算',
            settlementSummary: '今日核算状态',
            settlementCounts: '已核 {settled} · 待核 {unavailable} · 失败 {failed}',
            hint: '探针流水会标记为“智能探针”，不计入销售收入、真实流量或智能调度质量样本。',
            loadFailed: '探针费用配置加载失败',
            saveSuccess: '探针费用记账配置已保存',
            saveFailed: '探针费用记账配置保存失败'
          },
          accountStatuses: {
            none: '暂无状态'
          },
	        ...
	        poolErrorPolicy: {
	          title: '分组错误与重试策略',
	          description: '为当前分组统一覆盖池模式与错误码策略；未设置的字段继续继承账号或系统默认。不要把所有 502/503 自动视为官方过载。',
	          precedence: '优先级：分组显式配置 > 账号配置 > 系统默认；同一账号属于多个分组时按当前请求分组隔离。',
	          poolMode: '池模式开关',
	          retryCount: '同账号重试次数',
	          retryStatusCodes: '同账号重试状态码',
	          retryStatusCodesPlaceholder: '例如 529，多个用逗号分隔',
	          customEnabled: '自定义错误码处理',
	          customCodes: '自定义错误码',
	          customCodesPlaceholder: '例如 401、403，空白表示空列表',
	          inherit: '继承',
	          override: '覆盖',
	          enabled: '启用',
	          disabled: '停用',
	          inheritPlaceholder: '留空表示继承',
	          hint: '上游 529 已内置映射为客户端可重试的 503，并触发过载冷却；将 529 加入自定义错误码会改为停止该账号调度。',
	          saveSuccess: '分组错误与重试策略已保存'
	        },
	        formHint: '关闭后请求使用原调度；评分异常时会自动回退。',
        formEnabled: '已启用',
        formDisabled: '已关闭',
        requestedModel: '请求模型',
        requestedModelPlaceholder: '可选，例如 claude-sonnet-4-6',
        endpoint: '端点',
        anyEndpoint: '全部端点',
        chatCompletions: 'Chat Completions',
        responses: 'Responses',
        messages: 'Messages',
        geminiModels: 'Gemini 原生',
        refresh: '刷新预览',
        generatedAt: '生成于 {time}',
        algorithm: '算法 {version}',
        primary: '主候选',
        warm: '温备观察',
        isolated: '隔离/排除',
        total: '账号总数',
        ...
        confidence: '置信度',
        exploration: '探索预览',
        explorationRate: '探索 {rate}',
        explorationCandidate: '探索候选',
        probeBootstrap: '探针预热',
        fallbackEvidence: '回退证据',
        evidenceScopes: {
          model_endpoint: '模型 + 端点',
          model: '模型范围',
          endpoint: '端点范围',
          account: '账号全局'
        },
        ...
        load: '实时负载',
        decision: '调度结论',
        primaryCandidate: '主候选',
        observe: '观察池',
        excluded: '已排除',
        noScore: '未评分',
        noEvidence: '暂无真实流式证据',
        noAccounts: '该分组暂无绑定账号',
        noModel: '全部模型',
        failureSummary: '上游 {provider} · 暂态 {transient} · 限流 {rateLimit}',
        immediateFailureSummary: '近5分钟即时供应商失败：上游 {provider} · 暂态 {transient} · 限流 {rateLimit} · 待确认 {uncertain}',
        clientSummary: '用户侧 {client} · 平台 {platform} · 待确认 {uncertain}',
        capacityLimitedSummary: '最近1小时分组无可用容量事件：{count}。容量指标仅用于调度上下文，不计入账号质量分。',
        loadSummary: '{current}/{max} 并发 · 排队 {waiting} · {rate}%',
        noLoad: '暂无负载快照',
        loadSnapshotWarning: '负载数据未完整参与本次评分',
        confidenceLabels: { high: '高', medium: '中', low: '低' },
        poolLabels: { primary: '主候选', warm: '温备观察', isolated: '隔离/排除' },
        error: '智能调度预览加载失败'
      },
