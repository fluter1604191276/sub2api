export default {
    accounts: {
      ...
      modelSync: {
        title: '同步上游支持模型', summary: '检测到 {changed} 个账号有变化（共 {total} 个）', modeLabel: '同步策略', modeSync: '同步并移除已下架的自动白名单（保留手工映射）', modeAdd: '仅新增，不删除旧模型', apply: '应用选中 ({count})', applying: '应用中...', applied: '已应用 {count} 个账号，未完成 {conflicts} 个', applyFailed: '应用模型映射失败', statusApplied: '已应用', statusConflict: '账号已变化，请重新预览', statusFailed: '应用失败'
      },
      ...
      quality: {
        ...
        generationOnly: '首字证据不足，仅按生成 TPS 保守回退，最高 69 分',
        ttftOnly: '生成 TPS 证据不足，仅按首字保守回退，最高 79 分',
        hint: '基于最近 24 小时已计费且排除探测的流式流水，分别取最新 10 次和 100 次有耗时记录。评分 v3 先对每个分项按中位数 80%、尾部 20% 混合，再按首字 70%、生成 tokens/s 30% 合成；输出 token 少于 32 或生成时长不足 1000ms 的流水不计入 TPS 样本。账号、分组和调度评分使用同一数据公式，但因各自数据范围不同，结果可以不同。界面仍显示原始平均首字与总耗时。缺少 TPS 时使用最高 79 分的首字保守回退并明确标记；仅有生成 TPS 时最高 69 分。评级和评分仅供人工调度参考，不会自动修改账号优先级。',
