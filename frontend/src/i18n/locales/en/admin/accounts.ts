export default {
    accounts: {
      ...
      modelSync: {
        title: 'Sync upstream supported models', summary: '{changed} accounts changed ({total} total)', modeLabel: 'Sync strategy', modeSync: 'Sync and remove stale automatic whitelist entries (keep manual mappings)', modeAdd: 'Add new models only', apply: 'Apply selected ({count})', applying: 'Applying...', applied: 'Applied {count} accounts, {conflicts} incomplete', applyFailed: 'Failed to apply model mappings', statusApplied: 'Applied', statusConflict: 'Account changed; preview again', statusFailed: 'Apply failed'
      },
      ...
      quality: {
        ...
        generationOnly: 'TTFT evidence unavailable; generation-TPS fallback capped at 69',
        ttftOnly: 'Generation-TPS evidence unavailable; TTFT fallback capped at 79',
        hint: 'Uses billed, non-probe streaming usage rows from the last 24 hours and the latest 10/100 timed requests. Score v3 blends each component from 80% median and 20% tail, then combines TTFT at 70% with generation tokens/second at 30%. TPS excludes rows with fewer than 32 output tokens or under 1000 ms of generation time. The same formula is used for account, group, and scheduler scores, but each context can produce different results from its own rows. Original average TTFT and total duration remain displayed. Missing TPS uses a conservative TTFT-only fallback capped at 79 and is flagged; generation-only fallback is capped at 69. Grades and scores are for scheduling review only and never change account priority automatically.',
