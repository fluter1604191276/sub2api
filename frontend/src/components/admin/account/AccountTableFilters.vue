<template>
  <div class="flex flex-wrap items-center gap-3">
    ...
    <Select :model-value="filters.model" class="w-56" :options="mOpts" searchable @update:model-value="updateModel" @change="$emit('change')" />
  </div>
</template>
...
const props = defineProps<{ searchQuery: string; filters: Record<string, any>; groups?: AdminGroup[]; modelOptions?: SelectOption[] }>()
...
const mOpts = computed(() => [
  { value: '', label: t('admin.accounts.allModels') },
  ...(props.modelOptions || [])
])
</script>
