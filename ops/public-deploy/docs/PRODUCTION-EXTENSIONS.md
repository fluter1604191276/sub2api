...
| passive-capability-observation | Passive account-level observation of tool, client-tool, and terminal capability from real traffic, with an admin-only account summary column | account_capability.go, usage_log_repo_capability.go, account capability route, AccountsView.vue, capability tests | Operator visibility only; no billing, scoring, scheduling, probe, or account-state effect | Required |
...
| pricing-calibration | Channel/model pricing calibration, explicit billing boundaries, platform-aware display currency, and account-cost profile coverage audit | channel calibration service/repository/tests, pricing routes, platform currency helper, catalog UI tests, audit_account_stats_cost_coverage.py | User charges, internal cost, loss risk | Required |
...
| channel-monitor-budget | Durable Beijing-calendar daily base-price bills separate from budget reservations; configurable application-timezone cap; PostgreSQL-safe typed reservations and fail-closed invalid amount handling | channel_monitor_bills.go, MonitorDailyBills.vue, migration 234, extensions/20260910-monitor-daily-bills.md, service/handler/settings/UI | Probe spend and monitoring availability | Required |
| smart-probe-modes | Group sticky-escape policy presets, configurable primary score threshold, and high-frequency streaming recovery probes with canonical monetary quantization before settlement | smart_sticky_policy.go, smart_scheduler_preview.go, recovery probe runner/service/UI, migrations and tests | Account selection, probe load, upstream cost | Required |
| account-model-sync-preview | Preview and selectively apply upstream-supported model changes across accounts; atomic mapping/snapshot writes and fixed batch mode | AccountTestService, account_repo.go, admin account routes, AccountModelSyncDialog.vue, sync tests; extensions/20260910-model-sync-atomic-apply.md | Model availability and mapping | Required |
