...
| channel-monitor-budget | Daily estimated channel-monitor cost display and configurable application-timezone cap; PostgreSQL-safe typed reservations and fail-closed invalid amount handling | channel_monitor_daily_rollup.go, channel monitor service/handler/settings/UI, budget migration and tests | Probe spend and monitoring availability | Required |
